...
- 我想到的就是暴力解法，将原链表中的所有数组拷贝到数组中
- 使用`sort`函数对数组进行排序，数组将成为升序
- 然后再根据排序后的数组建立新的链表
...
- 使用dfs遍历整个二叉树
- 使用递归的方法实现dfs（递归这里还是不太熟悉）
- 共有两个递归
  - 一个求符合条件的路径的递归
    - 这个是在rootSum函数中进行递归
  - 一个是遍历二叉树的递归
    - 这个是在pathSum函数中进行递归，也就是主函数中
...
public:
    ...
    int pathSum(TreeNode* root, int targetSum) {
        ...
        // 用哈希表存储每个和对应有多少条路径
        // 采用dfs深度优先搜索
        // 对于每个根节点，我们都要探寻有没有这要一条合法的路径
        // 所以先调用探究合法路径的算法（一个递归）
        // 然后再调用每个根节点的算法，也就是主函数（两个递归）
        // 一共两个递归，能够实现找到所有路径
        if(!root)
            return 0;
...
- 回溯中使用dfs的要诀
  - 如果返回的是多个答案的二维vector
  - 那么在求取每个答案的时候，可以用用stl维护答案
  - 例如vector，string
  - 这样在dfs时就可以使用push_back()和pop_back操作修改和还原答案，这样的操作也符合y总的思想
  - 注意在回溯中，因为这道题不限制选择数字的个数
  - 所以当选择数字的情况时，继续调用函数时，index不用变化

```cpp
class Solution {
public:

    void backtrack(vector<vector<int>>& ans, vector<int>& combine, vector<int> candidates, int index, int target){
        ...
        // 不选择当前数字
        backtrack(ans, combine, candidates, index + 1, target);
        // 选择当前数字
        if(target - candidates[index] >= 0){
            combine.push_back(candidates[index]);
            backtrack(ans, combine, candidates, index, target - candidates[index]);
            combine.pop_back();
        }
    }
    vector<vector<int>> combinationSum(vector<int>& candidates, int target) {
        vector<vector<int>> ans;
        vector<int> combine;
        backtrack(ans, combine, candidates, 0, target);
        return ans;
...
- 这道题还有一种方法是使用完全背包
  - 使用了三维数组
  - 最外层表示和为`k`时，有多少数字组合，答案即为`dp[target]`
  - 依次枚举数组中的每一个数字`candidates[i]`
  - 然后枚举这个数和target之间的每一个数`j`
  - 取出所有和为`j - candidates[i]`的数组，并将`candidates[i]`push进去
  - 最后更新`dp[j]`
  - `dp[target]`即为答案
